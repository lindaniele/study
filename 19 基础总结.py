...
class Teacher(Grade,Banji):
    ...
    def run(self):
        print('我是老师%s，%s年级，%d班的%s老师'%(self.name,self.grades,int(self.banji),self.subject))
class Student(Grade,Banji):
    def __init__(self,name,age,grande_name,banji_name):
        Grade.__init__(self,grande_name)
        Banji.__init__(self,banji_name)
        self.name = name
        self.age = age
    def run(self):
        print('我是%s，%s年级，%d班的学生，今年%s岁'%(self.name,self.grades,self.banji,self.age))
grade = Grade('2')
print("年级"，grade)
banji = Banji('3')
print('班级'，Banji)
